...
export function rolloverDay(state: WellbeingState, now = Date.now()): WellbeingState {
...
export function startShift(now = Date.now()): WellbeingState {
  ...
  const next: WellbeingState = { ...state, shift: { startedAt: now, endedAt: null } };
  saveWellbeing(next);
  return next;
}

/** End the current shift, folding elapsed time into activeMs. */
export function endShift(reason: ShiftSession["endReason"] = "manual", now = Date.now()): WellbeingState {
  const state = rolloverDay(loadWellbeing(now), now);
  if (!state.shift || state.shift.endedAt !== null) return state;
...
export function addAfterActionNote(text: string, ref?: string, now = Date.now()): WellbeingState {
  const state = rolloverDay(loadWellbeing(now), now);
  ...
  const next: WellbeingState = { ...state, notes: [note, ...state.notes].slice(0, MAX_NOTES) };
  saveWellbeing(next);
  return next;
}
...
export function shortenMissionPath<TStep>(
  ...
  const reason = input.duressActive ? "duress" : quiet ? "quiet-hours" : "burnout";
