...
export interface FoiaRegistry {
  meta: { title: string; description: string; sources: string[]; note: string; last_updated: string };
...
export const DEFAULT_FOIA_REGISTRY: FoiaRegistry = {
  meta: {
    ...
    description: "Static jurisdiction templates for records-request generators.",
...
export function loadFoiaRegistry(raw: unknown): FoiaRegistry {
  if (raw && typeof raw === "object" && Array.isArray((raw as FoiaRegistry).templates)) {
...
export function listTemplates(): Array<{ id: string; jurisdiction: string; label: string }> {
...
export function isComplete(values: Record<string, string>, placeholders: string[]): boolean {
