...
export function classifyCrash(errorOrMessage: unknown): CrashCategory {
  ...
  if (/sign|verify|crypto|subtle|key|signature|ratchet/.test(lower)) return "crypto";
...
export function crashSummary(reports: CrashReport[]): Record<CrashCategory, number> {
