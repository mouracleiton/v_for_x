 ...
 */
export function measure(name: string, startName: string, endName: string): PerfMeasure | null {
  if (!enabled) return null;
  const start = marks.get(startName);
  const end = marks.get(endName);
  if (start === undefined || end === undefined) return null;
  const durationMs = Math.max(0, end - start);
  const m: PerfMeasure = { name, durationMs, startName, endName, ts: Date.now() };
...
export async function trace<T>(name: string, fn: () => Promise<T> | T): Promise<T> {
...
export function exportPerfJson(): string {
  return JSON.stringify({ exportedAt: Date.now(), marks: readPerfBuffer() }, null, 2);
}
