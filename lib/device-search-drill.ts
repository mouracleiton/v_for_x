...
export function scoreDeviceSearchDrill(input: DrillInput): DrillResult {
  ...
  const wipeCoverage = input.sensitiveKeys.length > 0 ? wipedSensitive / input.sensitiveKeys.length : 1;
  ...
  const decoyPlausibility = input.decoyKeys.length > 0 ? plausibleDecoys / input.decoyKeys.length : 0;

  // Time penalty: over-budget runs lose up to 20 points proportionally.
  const overage = withinBudget ? 0 : Math.min(1, (input.elapsedMs - budgetMs) / budgetMs);
  const timePenalty = overage * 20;
  const raw = wipeCoverage * 66 + decoyPlausibility * 34 - timePenalty;
  const score = Math.max(0, Math.min(100, Math.round(raw)));

  return { budgetMs, elapsedMs: input.elapsedMs, withinBudget, wipeCoverage, decoyPlausibility, score, missedKeys };
}
