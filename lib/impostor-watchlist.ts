...
export type WatchlistVerdict = "clean" | "new-handle" | "key-rotated" | "impostor-suspected";
...
export function saveWatchlist(data: Record<string, HandleKeyObservation[]>): void {
  if (typeof window === "undefined" || typeof localStorage === "undefined") return;
  try {
      ...
      findings: [
        {
          ...
          reason: "handle now uses a previously-seen alternate key (possible rotation)",
        },
      ],
    };
  }
    ...
    findings: [
      {
        ...
        reason: "handle presented a never-before-seen key; canonical key differs",
      },
    ],
  };
}
...
export function impostorBannerText(report: WatchlistReport, handle?: string): string {
