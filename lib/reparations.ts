...
export async function hashReparationsClaim(claim: ReparationsClaim): Promise<string> {
...
export async function verifyReparationsClaim(
  ...
  verifySignature: (canonical: string, publicKey: string, signature: string) => Promise<boolean>,
  ...
  const sigOk = await verifySignature(canonicalReparationsClaim(claim), publicKey, signature);
  if (!sigOk) {
...
export function parseReparationsClaim(token: string): SignedReparationsClaim | null {
  if (!token || !token.startsWith(REPARATIONS_PREFIX)) return null;
  try {
    const parsed = JSON.parse(token.slice(REPARATIONS_PREFIX.length));
    if (!parsed || typeof parsed !== "object" || !parsed.hash || !parsed.signature) return null;
...
export function appendReparationsClaim(claim: SignedReparationsClaim): SignedReparationsClaim[] {
