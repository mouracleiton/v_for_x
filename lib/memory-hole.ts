...
export function detectMemoryHoles(
  ...
  const verdict: MemoryHoleVerdict =
    missing.length > 0 ? "memory-hole" : changed.length > 0 ? "changed" : "clean";
...
export function memoryHoleBannerText(report: MemoryHoleReport): string {
  if (report.verdict === "unknown") return "Memory-hole check unavailable (offline).";
  if (report.verdict === "clean") return "Mirror matches seed — no deletions detected.";
