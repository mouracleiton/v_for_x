...
export const LOCAL_STORAGE_KEYS: ReadonlyArray<StorageKeyInfo> = [
  ...
  // ═════════════════════════════════════════════════════════════
  {
    key: "vfx_identity",
    category: "identity",
    description: "Current unified crypto identity (ECDSA P-256 keypair, handle, fingerprint)",
    sensitive: true,
    wipeOnPanic: true,
  ...
  },
  {
    ...
    category: "identity",
    description: "Previous identities for grace period verification (30-day signature validation)",
    sensitive: true,
    wipeOnPanic: true,
    preserveInDecoy: false,
    registeredAt: 1700000000000,
...
export const INDEXED_DB_STORES: ReadonlyArray<IndexedDBStoreInfo> = [
  ...
  // ═════════════════════════════════════════════════════════════
  { dbName: "vfx-store", storeName: "ledger", category: "user-data", description: "Trail ledger entries (mutual aid log)", sensitive: true },
  { dbName: "vfx-store", storeName: "checklists", category: "user-data", description: "Protocol X checklist kits", sensitive: true },
  { dbName: "vfx-store", storeName: "watchlist", category: "user-data", description: "Country watchlist entries", sensitive: true },
  { dbName: "vfx-store", storeName: "alert_rules", category: "user-data", description: "Custom metric alert rules", sensitive: true },
  { dbName: "vfx-store", storeName: "submissions", category: "content", description: "Anonymous dossier submissions", sensitive: true },
  { dbName: "vfx-store", storeName: "corroboration", category: "content", description: "Community corroboration network", sensitive: true },
  { dbName: "vfx-store", storeName: "action_circles", category: "content", description: "Action Network circles", sensitive: true },
  { dbName: "vfx-store", storeName: "pledges", category: "content", description: "Pledges (per-country)", sensitive: true },
  { dbName: "vfx-store", storeName: "dead_drops", category: "mesh", description: "Mesh dead-drop file envelopes", sensitive: true },
  { dbName: "vfx-store", storeName: "semantic_index", category: "cache", description: "Semantic search index cache", sensitive: false },
  { dbName: "vfx-store", storeName: "file_transfer", category: "mesh", description: "WebRTC file transfer state", sensitive: true },
  { dbName: "vfx-store", storeName: "crdt_docs", category: "content", description: "Collaborative CRDT documents", sensitive: true },
  { dbName: "vfx-store", storeName: "evidence_room", category: "crypto", description: "Evidence room hash chains", sensitive: true },
  { dbName: "vfx-store", storeName: "mirror_ring", category: "crypto", description: "Mirror ring state", sensitive: true },
  { dbName: "vfx-store", storeName: "mesh_store", category: "mesh", description: "Mesh store-and-forward mailbox", sensitive: true },

  // ═════════════════════════════════════════════════════════════
  // vfx-vault (encrypted evidence, media)
  // ═════════════════════════════════════════════════════════════
  { dbName: "vfx-vault", storeName: "items", category: "crypto", description: "Encrypted vault items (evidence, media)", sensitive: true },

  // ═════════════════════════════════════════════════════════════
  // vfx-offline (service worker cache, offline bundles)
  // ═════════════════════════════════════════════════════════════
  { dbName: "vfx-offline", storeName: "bundles", category: "cache", description: "Offline content bundles", sensitive: false },

  // ═════════════════════════════════════════════════════════════
  // vfx-pulse (alert feeds, notification state)
  // ═════════════════════════════════════════════════════════════
  { dbName: "vfx-pulse", storeName: "feeds", category: "user-data", description: "Alert feed subscriptions", sensitive: true },
  { dbName: "vfx-pulse", storeName: "state", category: "user-data", description: "Pulse notification state", sensitive: false },
] as const;
 ...
 */
export function getKeysForWipe(mode: "panic" | "duress" | "rotation"): string[] {
 ...
 */
export function getKeysForPreserve(mode: "panic" | "duress" | "rotation"): string[] {
  return LOCAL_STORAGE_KEYS.filter((info) => {
    if (mode === "duress") {
 ...
 */
export function getStoresForClear(mode: "panic" | "duress" | "rotation"): Array<{
  dbName: string;
  storeName: string;
}> {
...
export function findUnknownKeys(): string[] {
  ...
  const knownPrefixes = LOCAL_STORAGE_KEYS
    .map((info) => {
...
export async function executePanicWipe(
  ...
  const mode = trigger === "duress" ? "duress" : trigger === "rotation" ? "rotation" : "panic";
  ...
  if (backupKeys && backupLabel) {
    backup = createBackup(backupKeys, backupLabel, reason || "backup before wipe");
    saveBackup(backup);
  }
 ...
 */
async function clearIndexedDBStore(dbName: string, storeName: string): Promise<void> {
...
export function getStorageHealthReport(): {
  ...
  if (estimatedSize > 4_000_000) {
    warnings.push(`LocalStorage approaching quota limit (~${Math.round(estimatedSize / 1024 / 1024)}MB)`);
  }

  // Check for excessive audit events
  if (auditLog.length > 50) {
    warnings.push(`Audit log has ${auditLog.length} events (consider clearing)`);
  }
