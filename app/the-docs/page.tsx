...
async function sha256HexFirst8(text: string): Promise<string> {
...
export default function TheDocsPage() {
  ...
  const openDocById = useCallback(
    (id: string) => {
      ...
      const doc =
        raw && raw.startsWith(CRDT_PREFIX) ? CRDTDoc.decode(raw) : new CRDTDoc(actor || "actor", id);
      docRef.current = doc;
  ...
  const touchMeta = useCallback((id: string, docTitle: string, save: boolean) => {
    const meta = readJSON<DocMeta[]>(DOCS_KEY, []);
    const existing = meta.find((m) => m.id === id);
    const next: DocMeta[] = existing
      ? meta.map((m) => (m.id === id ? { ...m, title: docTitle, updatedAt: Date.now() } : m))
  ...
  const broadcast = useCallback(() => {
    ...
    busRef.current.postMessage({ type: "vfx-docs", token: doc.encode(), docId: doc.docId });
    sound.select();
  ...
  useEffect(() => {
    try {
      ...
      bus.onmessage = (ev: MessageEvent) => {
        const data = ev.data as { type?: string; token?: string; docId?: string };
  ...
  const deleteDoc = useCallback(() => {
    ...
    const meta = readJSON<DocMeta[]>(DOCS_KEY, []).filter((m) => m.id !== activeId);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs">
        ...
        <StatusPill color="dim">OPS {docRef.current?.getVersion() ?? 0}</StatusPill>
        ...
        <div className="space-y-4">
          <TerminalCard title="EDITOR" glow>
            ...
            <p className="text-[11px] text-content-dim mt-2">
              OPS {docRef.current?.getVersion() ?? 0} · CHARS {text.length} · CONFLICTS RESOLVED BY
              (LAMPORT, ACTOR) — NEWER WINS AT THE DIVERGENCE POINT
            </p>
          </TerminalCard>
          ...
          <TerminalCard title="HOW THIS WORKS">
            <p className="text-xs text-content-secondary leading-relaxed">
              SYNC HAPPENS WHEN DOCUMENTS MEET. EXPORT A TOKEN, SEND IT VIA THE WEB, A DEAD DROP,
              OR A BROADCASTCHANNEL — THE OTHER SIDE IMPORTS AND THE DOCS CONVERGE. NO SERVER, NO
              ACCOUNTS, NO CLOUD.
            </p>
            <p className="text-xs text-content-secondary leading-relaxed mt-2">
              EVERY CHARACTER IS AN OPERATION IN A REPLICATED GROWABLE ARRAY. CONCURRENT EDITS TO
              THE SAME SPOT RESOLVE DETERMINISTICALLY — NEWER OP WINS, TOMBSTONES NEVER RESURRECT —
              SO ANY TWO COPIES THAT EXCHANGE OPS REACH THE SAME TEXT.
            </p>
          </TerminalCard>
        </div>
      </div>
