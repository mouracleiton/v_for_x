...
export default function OfflineBriefcase() {
  ...
  const [status, setStatus] = useState<{ urlsCached: number; bytes: number } | null>(null);
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      if (data.type === "VFX_BUNDLE_PROGRESS") {
        setProgress({ done: data.done ?? 0, total: data.total ?? 0, bytes: data.bytes ?? 0 });
        setRunning(true);
      } else if (data.type === "VFX_BUNDLE_DONE") {
        setProgress({ done: data.urls ?? 0, total: data.urls ?? 0, bytes: data.bytes ?? 0 });
        setRunning(false);
        setDone(true);
        setStopped(false);
        sound.success();
  ...
  return (
    <TerminalCard title="OFFLINE BRIEFCASE" accent="green" glow className="mb-6">
      <p className="text-xs text-content-secondary mb-3">
        Crawl the whole platform into this device&apos;s cache. Once complete, the site runs with zero
        connectivity — from a USB stick, through an outage, under blackout. All pages stay local:
      ...
      {running && (
        <div className="mb-3">
          <div className="h-1.5 bg-panel border border-border-dim">
            <div className="h-full bg-terminal-green" style={{ width: `${percent}%` }} />
          </div>
      ...
      {(done || stopped || message) && (
        <div className="text-[11px] mb-2 flex flex-wrap items-center gap-2">
          <StatusPill color={stopped ? "amber" : "green"}>{message}</StatusPill>
        </div>
      )}
      ...
      <p className="text-[10px] text-content-dim">
        ▸ The crawl is best-effort: the core 200-country dataset and all page shells are included;
        very large media may be skipped. For a physical copy, download the build&apos;s{" "}
        <span className="text-terminal-green">out/</span> directory and serve it with{" "}
        <span className="text-terminal-green">npx serve</span>.
      </p>
    </TerminalCard>
  );
}
