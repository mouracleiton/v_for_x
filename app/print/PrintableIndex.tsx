    ...
    <main className="min-h-dvh max-w-6xl mx-auto px-3 sm:px-5 py-8">
      ...
      <div className="no-print mb-6">
        ...
        <p className="text-sm text-content-secondary max-w-3xl">
          Data-dense, print-ready intelligence briefs for all {data.countries.length}{" "}
          countries. Each brief carries key metrics, crisis indicators, a
          vulnerability breakdown, campaign needs, and dossier references —
          designed to be printed or saved as PDF/HTML for offline field use.
        </p>
      </div>

      <TerminalCard title="BRIEF INDEX" accent="green" className="mb-6 no-print">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center mb-5">
          <div>
            <div className="text-3xl font-bold text-terminal-green">{data.countries.length}</div>
          ...
          <div>
            <div className="text-3xl font-bold text-content-primary">{REGIONS.length}</div>
        ...
        {byRegion.map(([regionName, rows]) => (
          <section key={regionName}>
            <h2 className="text-xs uppercase tracking-widest text-content-secondary border-b border-border-dim pb-2 mb-3">
              ▸ {regionName} · <span className="text-content-dim">{rows.length}</span>
            </h2>
      ...
      <div className="print-only">
        <div className="text-center mb-4">
          <div className="text-[10px] uppercase tracking-widest text-gray-500">V FOR X // BRIEF INDEX</div>
          <h2 className="text-2xl font-bold">Printable Country Briefs</h2>
          <div className="text-xs text-gray-600 mt-1">
            {filtered.length} countries · generated {new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}
          </div>
        </div>
