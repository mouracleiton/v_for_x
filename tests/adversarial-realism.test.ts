...
describe("impostor-handle watchlist", () => {
  ...
  it("banner copy for impostor mentions re-verify", () => {
    const report = evaluate({ "lex-mira": [{ handle: "lex-mira", publicKey: "KEY-A", firstSeen: 1, lastSeen: 1 }] }, "lex-mira", "KEY-B");
...
describe("device-search drill", () => {
  it("perfect score when all sensitive wiped + decoys present + within budget", () => {
    const store: Record<string, string> = { decoy1: "real-looking", decoy2: "data" };
...
describe("compartmented crash reports", () => {
  ...
  it("scrubs keys, hashes, emails, and tokens", () => {
    const raw = "sign failed for key abc123def456... email a@b.com token VFXID1:secret123";
  ...
  it("records + persists a scrubbed crash report", () => {
    const report = recordCrash(new Error("signature verify failed for deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdead"));
