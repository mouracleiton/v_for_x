...
describe("EmptyState component", () => {
  ...
  it("renders the title in an accessible status region", () => {
    const { getByRole, getByText } = render(<EmptyState title="No witness statements yet" />);
  ...
  it("renders the hint when provided", () => {
    const { getByText } = render(
      <EmptyState title="Empty" hint="Record your first statement to start the chain." />,
    );
    expect(getByText("Record your first statement to start the chain.")).toBeTruthy();
  });
  ...
  it("renders an optional CTA node", () => {
    ...
    expect(getByText("Add statement").getAttribute("href")).toBe("/the-receipts");
  });
