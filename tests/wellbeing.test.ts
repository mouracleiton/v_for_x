...
describe("after-action notes", () => {
  ...
  it("addAfterActionNote stores a trimmed, capped note", () => {
    const state = addAfterActionNote("  Reflect on the op  ", "mission-1", 1_000);
  ...
  it("notes are most-recent-first and capped", () => {
    for (let i = 0; i < 5; i++) addAfterActionNote(`note ${i}`, undefined, 1_000 + i);
...
describe("shortenMissionPath", () => {
  it("returns full path when no stressor active", () => {
    const steps = ["a", "b", "c"];
    const out = shortenMissionPath({ steps, optionalStep: (_s, i) => i === 2 }, 1_000);
