...
describe("court bundle exporter", () => {
  ...
  it("includes witness + fulfillment rows when provided", () => {
    const html = renderCourtBundle({
      caption: "Test",
      witness: [
        { hash: "a".repeat(64), handle: "lex", text: "I saw it", ts: 1, prevHash: undefined, signature: "sig" },
      ],
      fulfillments: [
        { id: "f1", matchId: "m1", status: "delivered", note: "handoff", ts: 2, signature: "sig2" },
      ],
    });
...
describe("reparations claims ledger", () => {
  ...
  it("verifyReparationsClaim passes on consistent claim + good signature", async () => {
    const signed = await buildReparationsClaim(
      { claimant: "a", iso3: "usa", assetClass: "funds", description: "d", occurredAt: 1, filedAt: 2 },
      makeSigner(),
    );
  ...
  it("sumEstimatedValue aggregates", () => {
    const claims = [
      { id: "1", claimant: "a", iso3: "USA", assetClass: "funds", description: "d", occurredAt: 1, filedAt: 2, hash: "h1", publicKey: "p", signature: "s", estimatedValueUsd: 100 } as never,
      { id: "2", claimant: "a", iso3: "USA", assetClass: "land", description: "d", occurredAt: 1, filedAt: 2, hash: "h2", publicKey: "p", signature: "s", estimatedValueUsd: 250 } as never,
    ];
    expect(sumEstimatedValue(claims)).toBe(350);
  });

  it("canonicalReparationsClaim is stable", async () => {
    const c = { id: "x", claimant: "a", iso3: "USA", assetClass: "funds" as const, description: "d", occurredAt: 1, filedAt: 2 };
...
describe("FOIA / records-request generator", () => {
  ...
  it("generateFoiaRequest fills a complete request", () => {
    const req = generateFoiaRequest("USA", { topic: "border records", iso3: "MEX" });
