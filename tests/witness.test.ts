...
describe("canonicalWitnessContent / hashWitness", () => {
  it("serializes fields in the canonical order with iso3 as empty string", () => {
    ...
    expect(canonicalWitnessContent(stmt)).toBe(
      '{"id":"abc","text":"hello","iso3":"","ts":42,"prevHash":"' + "0".repeat(64) + '"}',
    );
  });
...
describe("buildWitness", () => {
  it("produces a deterministic hash for identical content", async () => {
    const input = { text: "deterministic", iso3: "SDN", ts: 1000, prevHash: "0".repeat(64) };
    ...
    const manual = { id: a.id, text: a.text, iso3: a.iso3, ts: a.ts, prevHash: a.prevHash };
  ...
  it("rejects empty text, over-long text, and malformed iso3", async () => {
    await expect(buildWitness({ text: "   " })).rejects.toThrow("must not be empty");
    await expect(buildWitness({ text: "x".repeat(MAX_WITNESS_TEXT + 1) })).rejects.toThrow(
      "exceeds",
    );
    await expect(buildWitness({ text: "ok", iso3: "US" })).rejects.toThrow("3-letter");
    await expect(buildWitness({ text: "ok", iso3: "US1" })).rejects.toThrow("3-letter");
  });
...
describe("signFn path", () => {
  it("produces a verifiable signature, anonymous by design", async () => {
    ...
    const stmt = await buildWitness({ text: "signed witness", iso3: "YEM" }, signFn);
    ...
    const unsigned = await verifyWitness({ ...stmt, signature: undefined, signerPublicKey: undefined, contentHash: undefined });
  ...
  it("fails verification when the signature is tampered", async () => {
    ...
    const bad = { ...stmt, signature: stmt.signature!.replace(/A/, "B") };
...
describe("encode / parse ledger", () => {
  it("round-trips a chain including signatures", async () => {
    const signFn = await createEphemeralSigner();
    const a = await buildWitness({ text: "round", iso3: "SDN", ts: 1000 }, signFn);
  ...
  it("rejects garbage tokens", () => {
    ...
    expect(() => parseWitnessLedger(LEDGER_PREFIX + "!!!not-base64!!!")).toThrow(/base64/i);
  ...
  it("rejects entries with missing required fields", () => {
    ...
    expect(() => parseWitnessLedger(LEDGER_PREFIX + b64)).toThrow(/required fields/i);
  });
...
describe("privacy helpers", () => {
  it("redactForPublic strips nothing — hashes are public", async () => {
    const signFn = await createEphemeralSigner();
    const stmt = await buildWitness({ text: "public by design", iso3: "HTI" }, signFn);
  ...
  it("produces a ZK blur proof that hides the member", async () => {
    ...
    const { proof, verified } = await zkProofForWitness(stmt, "hunger_hotspot", set);
  ...
  it("refuses to blur statements without a country or outside the set", async () => {
    ...
    await expect(zkProofForWitness(noCountry, "claim", ["SDN"])).rejects.toThrow(/no iso3/i);
...
describe("re-verification idempotence", () => {
  ...
  it("verifies an encoded→parsed chain identically", async () => {
    ...
    expect(await verifyWitnessChain(parsed)).toEqual(await verifyWitnessChain(chain));
  });
});